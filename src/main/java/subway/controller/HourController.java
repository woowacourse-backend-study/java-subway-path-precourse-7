...
public class HourController {
    ...
    public HourController(String startStation, String endStation) {
        initHourAndDistance();
        DijkstraShortestPath dijkstraShortestPath = new DijkstraShortestPath(hourGraph);
        List<String> shortestPath = dijkstraShortestPath.getPath(startStation, endStation).getVertexList();
        int shortestHour = (int) dijkstraShortestPath.getPath(startStation, endStation).getWeight();
        int totalDistance = checkDistance(shortestPath);
        OutputView.printResult(totalDistance, shortestHour, shortestPath);
    }

    private int checkDistance(List<String> shortestPath) {
        DijkstraShortestPath path = new DijkstraShortestPath(distanceGraph);
        int result = 0;
